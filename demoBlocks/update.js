function update () {
    ...
    //game.physics.arcade.collide(player1, player2); //Queremos que colisionen entre si los jugadores

    game.physics.arcade.collide(player1, player2, playerCollision, null, this);

    // colisiones con los blocks
    game.physics.arcade.collide(player1, blocks, blockCollision, null, this);
    game.physics.arcade.collide(player2, blocks, blockCollision, null, this);
...
// matar al jugador que toque el block
function blockCollision(player){
    player.kill();
}

//crear un nuevo block en posicion aleatoria
function fallBlock(){

    xCor = Math.floor(Math.random() * 800);

    blocks.create(xCor, 0, 'block');
